...
export default function WelcomeForm({ user, lang, dict }: { user: any, lang: string, dict: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // Trigger session update to refresh token data on client
        await update({ profileCompleted: true });
        router.push(`/${lang}`);
        router.refresh();
      } else {
        setError(result.debug ? `${result.error}: ${result.debug}` : (result.error || "Gagal menyimpan data"));
      }
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full py-4 bg-white text-black font-bold rounded-xl hover:bg-zinc-200 transition-all active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        {loading ? (
